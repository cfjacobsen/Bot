...
global.estrategia = estrategia;
  ...
  function obterSimboloString(simbolo, estadoPar = null) {
    try {
      ...
      console.error("Erro em obterSimboloString:", e);
      return 'ERROR';
    }
  }
...
// ====================== GERENCIAMENTO DE RISCO MULTI-CRYPTO ======================
...
class Ordem {
  ...
  if (config.SIMULA) {
    ...
    try {
      ...
      // 🔹 nome correto do parâmetro é "symbol"
      const body = {
        symbol: simbolo,
        side: ativoBase.toUpperCase(),
        type: tipo ? 'LIMIT' : 'MARKET',
        quantity: qtd,
        timestamp
      };
      ...
      if (preco) {
        body.price = preco;
        body.timeInForce = 'GTC'; // garante execução em ordem LIMIT
      }

      //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      console.log(`[PASSO] Passou por enviarOrdem 2  ${simbolo} <> ${body.side} <> ${body.type} <> ${timestamp} <> ${estado}- logger.iniciado: ${global.logger?.iniciado}`);
      //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%$%%%%%
      ...
      if (body.type === 'MARKET') {
        if (tipo === 'BUY') {
          body.quoteOrderQty = (qtd * estado.precoAtual).toFixed(2); // compra por valor
        } else {
          body.quantity = qtd.toFixed(precisao);
        }
      } else {
        body.quantity = qtd.toFixed(precisao);
        body.price = preco.toFixed(2);
        body.timeInForce = 'GTC';
      }

      const query = new URLSearchParams(body).toString();
      const signature = crypto.createHmac('sha256', config.API_SECRET).update(query).digest('hex');
      const url = `${getBinanceBaseUrl()}/api/v3/order?${query}&signature=${signature}`;

      //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      console.log(`[PASSO] Passou por enviarOrdem 3 => ${simbolo} <> ${body} <> ${url} - logger.iniciado: ${global.logger?.iniciado}`);
      //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%$%%%%%
      ...
      //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      console.log(`[PASSO] Passou por enviarOrdem 4 => ${simbolo} <> ${res.ok} - logger.iniciado: ${global.logger?.iniciado}`);
      //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%$%%%%%
      ...
      const data = await res.json();
      estado.healthCheck.ultimaConexao = new Date().toISOString();
      estado.ultimoPrecoExecutado = parseFloat(data.price) || estado.precoAtual;
      return data;
